...
This was a student project that grew organically, so a few things are worth knowing before you deploy your own instance:

- Session tokens currently don't expire — add a TTL check if you're deploying this for real users.
- Auth is passed as a query parameter (`?token=`) rather than an `Authorization` header — fine for a class project, not ideal for production.
- Review CORS settings before deploying publicly.

Contributions that harden these are very welcome.
...
Add your license of choice here (MIT is a common pick for student open-source projects).
